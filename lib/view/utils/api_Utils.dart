...
//         responseBody: true, responseHeader: true, request: true, requestBody: true, requestHeader: true, error: true));
//   }
//
...
//           deleteOnError: true, cancelToken: cancelToken ?? baseCancelToken, onReceiveProgress: downloadProgress);
...
//        // http.BaseResponse res = http.BaseResponse.fromJson(decoded, (data) => null);
...
//
//   static Future getApi({required url, required Function success, required failure}) async {
//     kPrintLog(url);
//
//     bool connected = await kInternetCheck();
//     if (!connected) {
...
//         }
//         // else{*/
//        // return failure!(res.message == null ? "" : res.message!); /*}*/
//       }
//     } on Exception catch (e) {
//       kPrintLog("Exception:${e.toString()}");
//       return failure(Strings.defaultExceptionMessage);
...
//       {required String? url, required Function success, required failure, required File file}) async {
//     kPrintLog(url);
//
//     bool connected = await kInternetCheck();
//     if (!connected) {
...
//
//       request.files.add(
//           http.MultipartFile.fromBytes("file", File(file.path).readAsBytesSync(), filename: file.path.split("/").last));
//
...
//       {required String? url, required Function success, required failure, required List<File> fileList}) async {
//     kPrintLog("$url");
//
//     bool connected = await kInternetCheck();
//
...
//         request.files.add(http.MultipartFile.fromBytes("photo[]", File(fileList[i].path).readAsBytesSync(),
//             filename: fileList[i].path.split("/").last, contentType: MediaType('jpg', 'png')));
//       }
//       var streamResponse = await request.send();
//       var response = await http.Response.fromStream(streamResponse);
//       Future.delayed(const Duration(seconds: 5), () {});
